...
def encode(x: Tensor) -> Tensor:
    """May take any arguments but must return exactly ONE tensor."""
    return x

def decode(x: Tensor) -> Tensor:
    """Inverse of encode on the receiving stage. Receives wire_unpack's ONE tensor."""
    return x
...
class GPTStage(nn.Module):
    def __init__(self, gpt: GPT, stage_index: int, num_stages: int):
        ...
        if self.is_first:
            self.embed, self.norm1 = gpt.embed, gpt.norm1
        ...
        if not self.is_first:   # raw (shape, dtype) on this stage's inbound boundary
            self.wire_meta = ((mbs, 1024, gpt.blocks[stage_index].attn.q.in_features), torch.bfloat16)

    def forward(self, x: Tensor, used: Tensor = None, targets: Tensor = None):
        if self.is_first:                # x: int32 token ids [64, 1024]
            x = self.norm1(self.embed(x))
...
for _ in range(num_trials):
    ...
    if stage_mod.is_first:
        adamw_groups.append(dict(params=[stage_mod.embed.weight], lr=0.7))
    if stage_mod.is_last:
        adamw_groups.append(dict(params=[stage_mod.proj.weight], lr=0.004))
    adamw_groups.append(dict(params=[p for p in stage_mod.parameters() if p.ndim < 2], lr=0.015))
